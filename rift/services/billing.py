...
async def _apply_checkout(result: dict, db: AsyncSession):
    ...
    # Always resolve user from Stripe customer ID, never trust metadata user_id.
    # Metadata can be forged in fake webhook payloads if webhook secret is ever leaked.
    stripe_session_id = result.get("stripe_session")
    if stripe_session_id:
        try:
            ...
            log.error(f"Could not verify webhook customer: {e}")
            return

    # Fallback to metadata user_id only if customer lookup found nothing
    # (e.g. new customer whose stripe_customer_id was just created this session)
    if user is None:
    ...
    if not user:
        log.warning(f"Webhook: could not resolve user for action={action}")
        return
